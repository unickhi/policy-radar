  ...
  <div class="flex flex-col h-full">
      ...
      <div class="flex justify-between items-center">
        <div class="flex gap-3 items-center">
          ...
          <el-select v-model="searchForm.status" placeholder="标准状态" clearable class="w-28">
            <el-option label="现行" value="现行" /><el-option label="即将实施" value="即将实施" />
          </el-select>
          ...
          <el-select v-model="searchForm.checkStatus" placeholder="核验状态" clearable class="w-28">
            <el-option label="待核验" :value="0" /><el-option label="已核验" :value="1" /><el-option label="不通过" :value="2" />
          </el-select>
    ...
    <div class="flex-1 bg-white overflow-hidden flex flex-col">
      <el-table :data="tableData" stripe v-loading="loading" border class="flex-1">
        ...
        <el-table-column prop="revision_type" label="制修订" width="80" />
        ...
        <el-table-column prop="approve_dept" label="批准部门" width="120" show-overflow-tooltip />
    ...
    <el-drawer v-model="drawerVisible" :title="drawerTitle" size="800px">
      <el-form :model="form" label-width="120px">
        <el-row :gutter="24">
          <el-col :span="12"><el-form-item label="标准号" required><el-input v-model="form.standard_no" placeholder="如：DL/T 5538-2025" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="标准名称" required><el-input v-model="form.standard_name" placeholder="请输入标准名称" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="24">
        ...
        <el-row :gutter="24">
          <el-col :span="12"><el-form-item label="制修订"><el-select v-model="form.revision_type" class="w-full"><el-option label="制定" value="制定" /><el-option label="修订" value="修订" /></el-select></el-form-item></el-col>
        ...
        <el-row :gutter="24">
          ...
          <el-col :span="12"><el-form-item label="标准类别"><el-input v-model="form.standard_class" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="12"><el-form-item label="技术归口"><el-input v-model="form.technical_owner" placeholder="如：电力行业电气工程施工及调试标准化技术委员会" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="批准部门"><el-input v-model="form.approve_dept" placeholder="如：国家能源局" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="12"><el-form-item label="行业分类"><el-input v-model="form.industry_class" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="代替标准"><el-input v-model="form.replace_standard" placeholder="被代替的标准号" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="12"><el-form-item label="中国分类号"><el-input v-model="form.ccs_code" placeholder="如：F21" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="国际分类号"><el-input v-model="form.ics_code" placeholder="如：27.100" /></el-form-item></el-col>
        </el-row>
        <el-form-item label="详情链接"><el-input v-model="form.detail_link" /></el-form-item>
...
import { industryApi, categoryApi } from '@/api/policy'
...
const searchForm = reactive({ keyword: '', status: '', checkStatus: undefined as number | undefined, categoryId: undefined as number | undefined })
const pagination = reactive({ page: 1, pageSize: 10 })

const drawerVisible = ref(false)
const drawerTitle = ref('新增政策')
const form = reactive<any>({ id: 0, standard_no: '', standard_name: '', publish_date: '', implement_date: '', revision_type: '制定', status: '现行', technical_owner: '', approve_dept: '', industry_class: '', standard_class: '', ccs_code: '', ics_code: '', replace_standard: '', detail_link: '', download_url: '', description: '', category_id: undefined as number | undefined })
...
const fetchData = async () => {
  loading.value = true
  try { const res = await industryApi.list({ ...pagination, ...searchForm }) as any; tableData.value = res.data.list; total.value = res.data.total }
...
const handleReset = () => { Object.assign(searchForm, { keyword: '', status: '', checkStatus: undefined, categoryId: undefined }); pagination.page = 1; fetchData() }

const handleAdd = () => {
  drawerTitle.value = '新增政策'
  Object.keys(form).forEach(key => form[key] = key === 'id' ? 0 : key === 'revision_type' ? '制定' : key === 'status' ? '现行' : '')
...
const handleSave = async () => {
  try { form.id ? await industryApi.update(form.id, form) : await industryApi.create(form); ElMessage.success('保存成功'); drawerVisible.value = false; fetchData() }
  catch (e: any) { ElMessage.error(e.message) }
}

const handleCheck = (row: any) => { checkForm.id = row.id; checkForm.check_status = 1; checkDrawerVisible.value = true }
const handleSaveCheck = async () => { try { await industryApi.check(checkForm.id, checkForm.check_status); ElMessage.success('核验完成'); checkDrawerVisible.value = false; fetchData() } catch (e: any) { ElMessage.error(e.message) } }

const handleDelete = (row: any) => {
  ElMessageBox.confirm('确定删除?', '提示', { type: 'warning' }).then(async () => { try { await industryApi.delete(row.id); ElMessage.success('删除成功'); fetchData() } catch (e: any) { ElMessage.error(e.message) } })
}
...
const handleFileChange = async (file: any) => {
  ...
  try {
    ...
    importPreview.value = data.map((row: any) => ({
      standard_no: row['标准号'] || '', standard_name: row['标准中文名称'] || '',
      publish_date: row['发布日期'] || '', implement_date: row['实施日期'] || '',
      revision_type: row['制修订'] || '制定', status: row['标准状态'] || '现行',
      technical_owner: row['技术归口'] || '', approve_dept: row['批准发布部门'] || '',
      industry_class: row['行业分类'] || '', standard_class: row['标准类别'] || '',
      ccs_code: row['中国标准分类号'] || '', ics_code: row['国际标准分类号'] || '',
      replace_standard: row['代替标准'] || '', detail_link: row['详情链接'] || '',
    }))
...
const handleDoImport = async () => {
  ...
  try { await industryApi.import(importPreview.value); ElMessage.success('导入成功'); importDrawerVisible.value = false; fetchData() }
